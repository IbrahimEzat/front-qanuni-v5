        ...
        <v-card>
            ...
            <v-container>
                ...
                <v-row class="justify-end">
                    ...
                    <v-btn color="primary ma-10" @click="updateBanner">تعديل لافتة</v-btn>
                </v-row>
            </v-container>
        </v-card>
    </v-container>
...
const { data, error } = await useSendRequest<responseReturn>(
    "/admin/banners/getBannerInfo",
    {
        token: authStore.token,
        banner_id: route.params.id
    }
)
...
if (error.value) {
    ...
    previewImage.value = banner.value.cover;
    banner_url.value = banner.value.url;
    banner_type.value = banner.value.type;
} else {
