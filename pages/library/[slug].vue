      ...
      >
        ...
        <div class="text-center">
          <v-btn
            variant="tonal"
            color="black"
            v-if="currentLibraries.length < searchFiles.length"
            @click="pageNumber++"
            >عرض المزيد</v-btn
          >
        </div>
    ...
    <v-dialog v-model="dialog" transition="dialog-top-transition" width="auto">
      <v-card elevation="4" class="text-center" max-width="300">
        <v-card-text class="text-h6">
          ...
          <div class="">
            أنت لا تستطيع إضافة ملف يجب عليك تسجيل الدخول أولا
          </div>
        </v-card-text>
...
const fillterLibraries = computed(() => {
  if (filterSelect.value === "newest") {
    ...
    return libraries.value.sort(function (a: any, b: any): number {
      if (a.library_view.views_count < b.library_view.views_count)
        return 1;
      return -1;
    });
    ...
    return libraries.value.sort(function (a: any, b: any): number {
      if (a.library_wish_list_count.wish_list_count < b.library_wish_list_count.wish_list_count)
        return 1;
      return -1;
    });
  }else if (filterSelect.value === "mostDownload") {
    return libraries.value.sort(function (a: any, b: any): number {
      if (a.library_download_count.download_count < b.library_download_count.download_count)
        return 1;
      return -1;
    });
...
const searchFiles = computed(()=>{
  if(search.value){
    return fillterLibraries.value.filter((library:any)=>{
      return library.file_name.toLowerCase().search(search.value.toLowerCase()) > -1;
    });
  }
  return fillterLibraries.value;
});

const currentLibraries = computed(() => {
  return searchFiles.value.slice(0, itemPerPage.value * pageNumber.value);
});

const { data, error } = await useSendRequest<responseReturn>("/library/get", {
...
if (error.value) {
  ...
  if (data.value?.status) {
    ...
    libraries.value = categoryInfo.value.libraries;
  }
}
...
useHead({
  title:'مكتبة - ' + categoryInfo.value.name,
  meta: [
    { name: "description", content: 'تحتوي هذه الصفحة على أهم المكتبات المتعلقة بتصنيف ' + categoryInfo.value.name },
    { property: "og:description", content: 'تحتوي هذه الصفحة على أهم المكتبات المتعلقة بتصنيف ' + categoryInfo.value.name  },
