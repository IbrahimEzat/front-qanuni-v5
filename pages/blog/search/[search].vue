<template>
  <v-card min-height="100vh" color="#f4f4f4" dir="rtl" class="">
    <v-row class="mx-1">
      <v-col md="2" sm="0">
      ...
      >
        ...
        <div class="text-center">
          <v-btn
            variant="tonal"
            color="black"
            v-if="currentBlogs.length < fillterBlogs.length"
            @click="pageNumber++"
            >عرض المزيد</v-btn
          >
        </div>
...
const currentBlogs = computed(() => {
  return fillterBlogs.value.slice(0, itemPerPage.value * pageNumber.value);
});

const { data, error } = await useSendRequest<responseReturn>("/blogs/search", {
