    ...
    <v-row class="">
      ...
      <v-col md="8" cols="12" class="bg-white my-8 pa-5">
        <v-row style="background-color: #fdf8ef; " class="mx-3 d-flex justify-space-between">
          <v-col sm="5" cols="12"  class="d-flex justify-space-between pe-xs-12 pe-1">
            ...
            <div class="imageHolder mt-2 pointer" @click="tryGoToAddDiscussion">
              <v-btn rounded="xl" prepend-icon="mdi-pencil-circle-outline" color="black">افتح مناقشة</v-btn>
            </div>
          </v-col>
        ...
        <div class="ma-3 pa-5">
          ...
          <v-alert v-if="discussionNumber == 0"  type="info">لا يوجد تصنيفات فالموقع</v-alert>
        </div>
      </v-col>
      <v-col md="2" sm="0">
        <bannar-slide></bannar-slide>
...
const categories =
  ref<{ name: string; color: string; background: string; slug: string;discussions_count: number; }[]>();
const { setToastMessage } = useSettingsStore();
const { data, error } = await useSendRequest<responseReturn>("/categories/CategotiesWithDiscussions", {});
if (error.value) {
...
const {data:countData,error:errorCount} = await useSendRequest<responseReturn>('/discussions/count',{});
...
useHead({
  ...
  meta: [
    { name: "description", content: 'تحتوي هذه الصفحة على التصنيفات الخاصة بأهم المناقشات' },
    { property: "og:description", content: 'تحتوي هذه الصفحة على التصنيفات الخاصة بأهم المناقشات' },
