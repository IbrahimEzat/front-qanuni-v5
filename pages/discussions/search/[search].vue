      ...
      >
        ...
        <div class="text-center">
          <v-btn
            variant="tonal"
            color="black"
            v-if="currentDiscussions.length < fillterDiscussions.length"
            @click="pageNumber++"
            >عرض المزيد</v-btn
          >
        </div>

        <v-alert class="ma-3" type="info" v-if="discussions.length === 0"
...
const currentDiscussions = computed(() => {
  return fillterDiscussions.value.slice(0, itemPerPage.value * pageNumber.value);
});
...
const { data, error } = await useSendRequest<responseReturn>(
  ...
  if (error.value) {
    ...
    if (data.value?.status) {
      discussions.value = data.value.data;
    }
  }
...
useHead({
  title:'مناقشات - نتائج بحث| ' + route.params.search,
  meta: [
