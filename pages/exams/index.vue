        ...
        <div class="ma-3 pa-5">
          <div class="d-flex flex-wrap align-center">
            <div
              ...
              :key="category.id"
              v-show="category.exams_count != 0"
            >
              <v-sheet
                @click="$router.push('/exams/' + category.slug)"
          ...
          <v-alert v-if="examsNumber == 0" type="info"
...
const { data, error } = await useSendRequest<responseReturn>(
  "/categories/CategotiesWithExams",
  {}
);
if (error.value) {
...
const { data: countData, error: errorCount } =
  await useSendRequest<responseReturn>("/exams/count", {});
...
useHead({
  ...
  meta: [
    {
      name: "description",
      content: "تحتوي هذه الصفحة على التصنيفات الخاصة بأهم الاختبارات",
    },
    {
