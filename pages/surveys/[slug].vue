      ...
      <v-col style="border: 1px solid #999" class="bg-white mb-5 mt-sm-8 pa-0 pb-16" md="8" sm="11" cols="12">
        <div style="background-color: #fdf8ef; width: 100%" class="pa-3">
          ...
          <div class="d-flex justify-space-between mt-5">
            ...
            <div class="d-flex">
              <v-select label="Select" style="min-width: 200px; height: 45px" variant="outlined" class="pa-0 ml-10"
                density="compact" v-model="filterSelect" item-title="name" item-value="value" :items="[
            ...
            <v-text-field v-model="search" clearable style="max-width: 200px; max-height: 9px" class="rounded-pill" label="ابحث"
              density="compact" prepend-inner-icon="mdi-magnify" variant="outlined"></v-text-field>
          </div>
        ...
        <v-card border class="mx-3 my-10 pa-5 d-flex" style="width: 97.5%; border-radius: 20px"
          v-for="survey in survyesSearch" :key="survey.id">
          <v-row class="pointer" @click="$router.push('/surveys/' + survey.id + '/show')">
            <v-col cols="12" md="10">
            <div class="d-md-none d-block">
              <div class="d-flex justify-space-between align-baseline">
                <h4>{{ survey.question_name }}</h4>
                <v-avatar v-if="questionAlreadyVoit && questionAlreadyVoit.includes(survey.id + '')
                  " size="90" color="success" style="border: 2px solid black" text=" asdad">
                  <div class="">
                    <v-icon icon="mdi-heart"></v-icon><br />أبديت رأيك
                  </div>
                </v-avatar>
                <v-avatar v-else size="90" color="white" style="border: 2px solid black">
                  لم تشارك بعد
                </v-avatar>
              </div>
              <div class="d-flex justify-space-between">
                ...
                <div class="d-flex">
                  ...
                  <div class="imageHolder ml-1 align-self-center" style="width: 15px; height: 15px">
                    <v-img src="/images/قائمة-المفضلة-قبل-الضغط.png" style="width: 100%; height: 100%"></v-img>
                  </div>
              ...
              <div class="d-md-block d-none">
                  <p class="text-h4 text-center ms-16">
            ...
            <v-col cols="0" md="2" align-self="center">
              <div class="d-md-block d-none">
                <v-avatar v-if="questionAlreadyVoit && questionAlreadyVoit.includes(survey.id + '')
                " size="90" color="success" style="border: 2px solid black" text=" asdad">
                <div class="">
        ...
        <div class="text-center">
          <v-btn variant="tonal" color="black" v-if="currentSurveys.length < survyesSearch.length" @click="pageNumber++">عرض
            المزيد</v-btn>
        </div>
...
const survyesSearch = computed(()=> {
  if(search.value){
    return fillterSurveys.value.filter((item :any)=>{
      return item.question_name.toLowerCase().search(search.value.toLowerCase()) > -1;
    });
  }
  return fillterSurveys.value
});
...
const currentSurveys = computed(() => {
  return survyesSearch.value.slice(0, itemPerPage.value * pageNumber.value);
});

const { data, error } = await useSendRequest<responseReturn>("/surveys/index", {
...
if (error.value) {
  ...
  if (data.value?.status) {
    ...
    surveys.value = categoryInfo.value.surveys.reverse();
