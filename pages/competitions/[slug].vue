        ...
        <v-row class="mx-1">
            ...
            <v-col style="border: 1px solid #999" class="bg-white mb-5 mt-sm-8 pa-0 pb-16" md="8" sm="11" cols="12">
                <div style="background-color: #fdf8ef; width: 100%" class="pa-3">
                    <div class="d-flex justify-space-between">
                        <span class="font-weight-bold pt-2 ml-5"><nuxt-link class="text-decoration-none font-weight-bold"
                            ...
                            <span>/{{ route.params.slug }} </span></span>
                    ...
                    <div class="d-flex justify-space-between mt-5">
                        ...
                        <v-text-field clearable style="max-width: 200px; max-height: 9px" class="rounded-pill" label="ابحث"
                            density="compact" v-model="search" prepend-inner-icon="mdi-magnify"
                ...
                <div class="text-center">
                    <v-btn variant="tonal" color="black" v-if="currentCompetition.length < competitionSearch.length"
                        @click="pageNumber++">عرض المزيد</v-btn>
                </div>

                <v-alert class="ma-3" type="info" v-if="competitionSearch.length === 0">لا يوجد مسابقات لعرضها</v-alert>
...
const competitionSearch = computed(() => {
    if (search.value) {
        return fillterCompetition.value.filter((item) => {
            return item.title.toLowerCase().search(search.value.toLowerCase()) > -1;
        });
    }
    return fillterCompetition.value
});
...
const fillterCompetition = computed(() => {
    ...
    if (filterSelect.value == "mostFavioret") {
        return competitions.value.sort(function (a, b) {
            if (a.wishlists.length < b.wishlists.length) return 1;
            return -1;
        });
    } 
...
const currentCompetition = computed(() => {
    return competitionSearch.value.slice(0, itemPerPage.value * pageNumber.value);
});

const { data, error } = await useSendRequest(
    "/competitions/getAllCompetition",
...
if (error.value) {
    ...
    if (data.value?.status) {
        ...
        competitions.value = categoryInfo.value.competitions;
    }
}


useHead({
    title: 'مسابقات - ' + categoryInfo.value.name,
    meta: [
        { name: "description", content: 'تحتوي هذه الصفحة على أهم المسابقات المتعلقة بتصنيف ' + categoryInfo.value.name },
        { property: "og:description", content: 'تحتوي هذه الصفحة على أهم المسابقات المتعلقة بتصنيف ' + categoryInfo.value.name },
